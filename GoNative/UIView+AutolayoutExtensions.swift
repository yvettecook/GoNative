...
extension UIView {
    ...
    func limitToSuperviewTop(withRange range:(min: CGFloat, max: CGFloat), priority: UILayoutPriority = UILayoutPriorityRequired) -> (min: NSLayoutConstraint, max: NSLayoutConstraint) {
        guard let superview = self.superview else {
            preconditionFailure("view has no superview")
        }
    ...
    func limitToSuperviewBottom(withRange range:(min: CGFloat, max: CGFloat), priority: UILayoutPriority = UILayoutPriorityRequired) -> (min: NSLayoutConstraint, max: NSLayoutConstraint) {
        guard let superview = self.superview else {
            preconditionFailure("view has no superview")
        }
    ...
    func addMinHeightConstraint(withConstant constant: CGFloat, priority: UILayoutPriority = UILayoutPriorityRequired) -> NSLayoutConstraint {
        translatesAutoresizingMaskIntoConstraints = false
...
extension UIView {
    ...
    func alignVerticalCenterWithSuperview(withMultiplier multiplier: CGFloat = 1.0, priority: UILayoutPriority = UILayoutPriorityRequired) -> NSLayoutConstraint {
        let superview = unWrappedSuperview()

        let constraint = alignVerticalCenter(withView: superview, withMultiplier: multiplier)
        constraint.priority = priority
